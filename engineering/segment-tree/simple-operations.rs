...
// ==========================================================
// main - runs the three use case demos
// ==========================================================
fn main() {

    // ------------------------------------------------------
    // Use case 1: grades (MAX)
    // ------------------------------------------------------
    let grades: Vec<i32> = vec![7, 3, 9, 5, 8, 2, 6, 4];
    let mut st = SegmentTree::new(grades, -1, merge_max);

    println!("==============================================");
    println!("Use case 1: notas da turma (MAX)");
    println!("==============================================");
    println!("Notas dos 8 alunos: {:?}", st.data);
    println!("Arvore montada. Maior nota da turma = {}", st.root());
    println!();
    print_tree(&st.tree, st.len());

    println!("Caso 1 - maior nota entre os alunos 3 e 6");
    println!("  Segment tree responde: {}", st.query(2, 5));
    println!("  Conferindo na mao:    max de {:?} = {}", &st.data[2..6], st.data[2..6].iter().max().unwrap());
    println!();

    println!("Caso 2 - aluno 5 refez a prova: nova nota = 10");
    st.update(4, 10);
    println!("  Notas apos atualizacao: {:?}", st.data);
    println!("  Nova maior nota da turma: {}", st.root());
    println!();
    ...
    // ------------------------------------------------------
    // Use case 2: daily sales (SUM)
    // ------------------------------------------------------
    let sales: Vec<i32> = vec![4, 1, 3, 5, 2, 6, 1, 2];
    let mut st = SegmentTree::new(sales, 0, merge_sum);
    ...
    println!("==============================================");
    println!("Vendas dos 8 dias: {:?}", st.data);
    println!("Arvore montada. Soma total = {}", st.root());
    println!();

    println!("Caso 1 - soma das vendas do dia 3 ao dia 6");
    println!("  Segment tree responde: {}", st.query(2, 5));
    let manual_sum: i32 = st.data[2..6].iter().sum();
    println!("  Conferindo na mao:     {} + {} + {} + {} = {}",
        st.data[2], st.data[3], st.data[4], st.data[5], manual_sum);
    println!();

    println!("Caso 2 - corrigindo o dia 4 de 5 para 7");
    st.update(3, 7);
    println!("  Vendas apos correcao: {:?}", st.data);
    println!("  Nova soma total:      {}", st.root());
    println!();

    println!("Caso 3 - soma do dia 3 ao dia 6 (depois da correcao)");
    println!("  Segment tree responde: {}", st.query(2, 5));
    let manual_sum: i32 = st.data[2..6].iter().sum();
    println!("  Conferindo na mao:     {}", manual_sum);


    // ------------------------------------------------------
    // Use case 3: oxygen saturation (MIN)
    // ------------------------------------------------------
    let spo2: Vec<i32> = vec![98, 97, 92, 94, 96, 99, 95, 93];
    let mut st = SegmentTree::new(spo2, INF, merge_min);
    ...
    println!("  {:?}", st.data);
    println!("Arvore montada. Menor saturacao do dia = {} %", st.root());
    println!();

    println!("Caso 1 - menor saturacao entre 10h e 16h");
    println!("  Segment tree responde: {}", st.query(2, 5));
    println!("  Conferindo na mao:    min de {:?} = {}", &st.data[2..6], st.data[2..6].iter().min().unwrap());
    println!();

    println!("Caso 2 - corrigindo medicao das 10h: de 92 para 90");
    st.update(2, 90);
    println!("  SpO2 apos correcao:           {:?}", st.data);
    println!("  Menor saturacao do dia agora: {}", st.root());
    println!();
